...
function resolvePlugin() {
  if (
    ...
    typeof uni.requireNativePlugin !== "function"
  ) {
    return null;
  }
...
export function listTileDirectories(path) {
  const plugin = resolvePlugin();
  const value = String(path || "/storage").trim() || "/storage";

  if (!plugin || typeof plugin.listDirectories !== "function") {
  ...
  return new Promise((resolve) => {
    try {
      plugin.listDirectories({ path: value }, (result) => {
        resolve({
          success: Boolean(result && result.success),
          message: result && result.message ? result.message : "",
          currentPath:
            result && result.currentPath ? result.currentPath : value,
          parentPath: result && result.parentPath ? result.parentPath : "",
          directories: result && result.directories ? result.directories : [],
        });
      });
    } catch (error) {
      console.warn("[TileServer] listDirectories 调用失败:", error);
      resolve({
        success: false,
